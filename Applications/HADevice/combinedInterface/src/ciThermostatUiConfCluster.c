/**************************************************************************//**
  \file ciThermostatUiConfCluster.c
  ...
  uint16_t attr, AttibuteReadCallback_t cb)
{
  ...
  fillDstAddressing(&req->dstAddressing, mode, addr, ep, THERMOSTAT_UI_CONF_CLUSTER_ID);
...
static void ZCL_ReadAttributeResp(ZCL_Notify_t *ntfy)
{
  ...
  if (ZCL_SUCCESS_STATUS == ntfy->status)
  {
    ...
    LOG_STRING(readAttrSuccessStr, "<-Read Thermostat UI conf attribute (0x%x) response: success  t = 0x%x\r\n");
    appSnprintf(readAttrSuccessStr, (unsigned)readAttributeResp->id, attributeValue);
  }
  else
  {
    LOG_STRING(readAttrFailStr, " +Read Thermostat UI conf attribute failed: status = 0x%2x\r\n");
...
static void ZCL_WriteAttributeResp(ZCL_Notify_t *ntfy)
{
  if (ZCL_SUCCESS_STATUS == ntfy->status)
  {
    ...
    LOG_STRING(writeAttrSuccessStr, " <-Write Thermostat UI conf attribute response: success\r\n");
    appSnprintf(writeAttrSuccessStr);
  }
  else
  {
    LOG_STRING(writeAttrFailStrTwo, " +Write Thermostat UI conf attribute failed: status = 0x%x\r\n");
    appSnprintf(writeAttrFailStrTwo, (unsigned)ntfy->status);
  }
}
#endif // APP_DEVICE_TYPE_COMBINED_INTERFACE

// eof ciThermostatUiCluster.c

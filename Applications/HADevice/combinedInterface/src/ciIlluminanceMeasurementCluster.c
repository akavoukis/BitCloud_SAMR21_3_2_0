/**************************************************************************//**
  \file ciIlluminanceMeasurementCluster.c

  \brief
    Combined Interface Illuminance Measurement Cluster implementation.
...
{
  ZCL_Cluster_t *cluster = ZCL_GetCluster(APP_SRC_ENDPOINT_ID, ILLUMINANCE_MEASUREMENT_CLUSTER_ID, ZCL_CLUSTER_SIDE_CLIENT);
  ...
  uint16_t attr, AttibuteReadCallback_t cb)
{
  ...
  fillDstAddressing(&req->dstAddressing, mode, addr, ep, ILLUMINANCE_MEASUREMENT_CLUSTER_ID);
...
static void ciLightSensorReportInd(ZCL_Addressing_t *addressing, uint8_t reportLength, uint8_t *reportPayload)
{
  ...
  LOG_STRING(reportAttrIndStr, "<-Light Sensor Attr Report: t = %d\r\n");
...
static void ZCL_ReadAttributeResp(ZCL_Notify_t *ntfy)
{
  ...
  if (ZCL_SUCCESS_STATUS == ntfy->status)
  {
    ...
    LOG_STRING(readAttrSuccessStr, " <-Read Illuminance Measurement Attribute (0x%x) response: success\r\n  t = %u\r\n");
    appSnprintf(readAttrSuccessStr, (unsigned)readAttributeResp->id, attributeValue);
  }
  else
  {
    LOG_STRING(readAttrFailStr, " +Read Illuminance Measurement Attribute failed: status = 0x%2x\r\n");
...
static void ZCL_WriteAttributeResp(ZCL_Notify_t *ntfy)
{
  if (ZCL_SUCCESS_STATUS == ntfy->status)
  {
    ...
    LOG_STRING(writeAttrSuccessStr, " <-Write Illuminance Measurement attribute response: success\r\n");
    appSnprintf(writeAttrSuccessStr);
  }
  else
  {
    LOG_STRING(writeAttrFailStrTwo, " +Write Illuminance Measurement attribute failed: status = 0x%x\r\n");

/**************************************************************************//**
  \file ssIdentifyCluster.c

  \brief
    Dimmer Switch Identify cluster implementation.
...
static void dsFillIdentifyQueryResponsePayload(ZCL_IdentifyQueryResponse_t *payload, uint16_t time);
static void dsFillUpdateCommissioningStatePayload(ZCL_UpdateCommissioningState_t *payload, uint8_t action, uint8_t mask);
...
******************************************************************************/
ZCL_IdentifyClusterAttributes_t dsIdentifyClusterServerAttributes =
{
  ZCL_DEFINE_IDENTIFY_CLUSTER_SERVER_ATTRIBUTES()
};
ZCL_IdentifyClusterCommands_t   dsIdentifyCommands =
{
...
{
  dsIdentifyClusterServerAttributes.commissioningState.value.networkState     = nwkState;
  dsIdentifyClusterServerAttributes.commissioningState.value.operationalState = operationalState;
}
...
void identifySendIdentifyQuery(APS_AddrMode_t mode, ShortAddr_t addr, Endpoint_t ep)
{
  ...
  fillDstAddressing(&req->dstAddressing, APS_SHORT_ADDRESS, addr, ep, IDENTIFY_CLUSTER_ID);
  ...
  ZCL_UpdateCommissioningStateAction_t action, uint8_t mask)
{
  ...
  dsFillUpdateCommissioningStatePayload((ZCL_UpdateCommissioningState_t *)req->requestPayload, action, mask);
  fillDstAddressing(&req->dstAddressing, APS_SHORT_ADDRESS, addr, ep, IDENTIFY_CLUSTER_ID);
  commandManagerSendCommand(req);
  (void)mode;
}
...
static ZCL_Status_t identifyInd(ZCL_Addressing_t *addressing, uint8_t payloadLength, ZCL_Identify_t *payload)
{
  ...
  dsIdentifyClusterServerAttributes.identifyTime.value = payload->identifyTime;
...
static ZCL_Status_t updateCommissioningStateInd(ZCL_Addressing_t *addressing, uint8_t payloadLength, ZCL_UpdateCommissioningState_t *payload)
{
  ...
  {
    dsIdentifyClusterServerAttributes.commissioningState.value.networkState     |= payload->commissioningStateMask.networkState;
    dsIdentifyClusterServerAttributes.commissioningState.value.operationalState |= payload->commissioningStateMask.operationalState;
  }
...
static ZCL_Status_t sendIdentifyQueryResponse(ZCL_Addressing_t *addressing)
{
  ...
  dsFillIdentifyQueryResponsePayload((ZCL_IdentifyQueryResponse_t *)req->requestPayload, dsIdentifyClusterServerAttributes.identifyTime.value);

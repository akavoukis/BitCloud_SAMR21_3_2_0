/**************************************************************************//**
  \file dsAlarmCluster.c

  \brief
    DimmerSwitch Alarm cluster implementation.
...
******************************************************************************/
ZCL_AlarmsClusterServerAttributes_t dsAlarmsClusterServerAttributes =
{
  ZCL_DEFINE_ALARMS_CLUSTER_SERVER_ATTRIBUTES()
};
ZCL_AlarmsClusterServerCommands_t dsAlarmsClusterServerCommands =
{
...
******************************************************************************/
ZCL_AlarmEntry_t dsAlarmTable[ALARM_TABLE_SIZE];
...
{
  dsAlarmsClusterServerAttributes.alarmCount.value = 0;
}
...
static ZCL_Status_t getAlarmInd(ZCL_Addressing_t *addressing, uint8_t payloadLength, uint8_t *payload)
{
  ...
  if (dsAlarmsClusterServerAttributes.alarmCount.value)
  {
    reqLength = sizeof(ZCL_getAlarmResponse_t);
    getAlarmResponse->status = ZCL_SUCCESS_STATUS;
    getAlarmResponse->alarmCode = dsAlarmTable[0].alarmCode;
    getAlarmResponse->clusterIdentifier = dsAlarmTable[0].clusterIdentifier;
    getAlarmResponse->timeStamp = dsAlarmTable[0].timeStamp;
    removeAlarmEntry();
  }
  else
  {
...
static void addAlarmEntry(uint8_t alarmCode, ClusterId_t clusterId)
{
  if (dsAlarmsClusterServerAttributes.alarmCount.value >= ALARM_TABLE_SIZE)
    removeAlarmEntry();

  dsAlarmTable[dsAlarmsClusterServerAttributes.alarmCount.value].alarmCode = alarmCode;
  dsAlarmTable[dsAlarmsClusterServerAttributes.alarmCount.value].clusterIdentifier = clusterId;
  dsAlarmTable[dsAlarmsClusterServerAttributes.alarmCount.value].timeStamp = HAL_GetSystemTime() / 1000; // this needs to be linked with time cluster UTC
  dsAlarmsClusterServerAttributes.alarmCount.value++;
...
static void removeAlarmEntry()
{
  for (uint8_t i = 0; i < (dsAlarmsClusterServerAttributes.alarmCount.value-1); i++)
    memcpy(&dsAlarmTable[i], &dsAlarmTable[i+1], sizeof(ZCL_AlarmEntry_t));
  dsAlarmsClusterServerAttributes.alarmCount.value--;
}

#endif // APP_DEVICE_TYPE_DIMMER_SWITCH

// eof dsAlarmsCluster.c

...
void halStartSleepTimerClock(void)
{
  ...
  GCLK_GENCTRL_s.id = 2;      //generator 2
  GCLK_GENCTRL_s.src = 0x04;  // OSC832K
  GCLK_GENCTRL_s.divsel = 0;  // no division so 32KHz clock
  GCLK_GENCTRL_s.genen = 1;
...
*/
 ...
 /* Ebanble RTC APB CLK in the power manager */ 
  PM_APBAMASK |= (1 << 5);

  /* Enable the generic clk to clk the RTC */
  GCLK_CLKCTRL_s.id = 0x04;  // enabling clock for RTC

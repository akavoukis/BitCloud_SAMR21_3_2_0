/************************************************************************//**
  ...
  Last change:
    $Id: zclOtauClient.c 27608 2015-01-23 16:39:58Z srinivasan.ganapathy $
******************************************************************************/
...
#if APP_SUPPORT_OTAU_RECOVERY == 1
  if (PDS_IsAbleToRestore(OTAU_PDT_MEMORY_MEM_ID))
  {
    PDS_Restore(OTAU_PDT_MEMORY_MEM_ID);
...
#if APP_SUPPORT_OTAU_RECOVERY == 1
      PDS_Store(OTAU_PDT_MEMORY_MEM_ID);
#endif
    ...
    case ZCL_SUCCESS_STATUS: // received on aps ack / low-layer confirmation
      ...
      if (IMAGE_PAGE_REQUEST_ID == tmpZclReq->id)
      {
        ...
        if (pageRequestUsed && (IMAGE_PAGE_REQUEST_ID == tmpZclReq->id))
        {
          ...
          tmpPageReqTimer->interval  = WAITING_BLOCK_RESPONSE_TIMEOUT;
...
void zclOtauFillOutgoingZclRequest(uint8_t id, uint8_t length, uint8_t *payload)
{
  ...
  rspWaitTime = (OTAU_MAX_RESPONSE_WAIT_TIMEOUT < rspWaitTime)?OTAU_MAX_RESPONSE_WAIT_TIMEOUT:rspWaitTime;

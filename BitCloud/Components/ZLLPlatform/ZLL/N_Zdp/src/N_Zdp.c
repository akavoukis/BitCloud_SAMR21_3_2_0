...
static union
{
  ...
  struct PACK
  {
    ...
    N_Zdp_NeighborTableRecord_t list[ZDP_NEIGBOR_TABLE_LIST_SIZE - 1U];
  } mgmtLqi;
...
static void zdpMgmtLqiResp(ZDO_ZdpResp_t* zdpResp)
{
  if (zdpIsZigBeeStandardStatus(zdpResp->respPayload.status))
  {
    ...
    dstRsp->startIndex = srcRsp->startIndex;
    N_ERRH_ASSERT_FATAL( srcRsp->neighborTableListCount <= ZDP_NEIGBOR_TABLE_LIST_SIZE );
    dstRsp->neighborTableListCount = srcRsp->neighborTableListCount;
...
static void zdpMgmtRtgResp(ZDO_ZdpResp_t* zdpResp)
{
  if (zdpIsZigBeeStandardStatus(zdpResp->respPayload.status))
  {
    ...
    dstRsp->startIndex = srcRsp->startIndex;
    N_ERRH_ASSERT_FATAL( srcRsp->routingTableListCount <= ZDP_ROUTING_TABLE_LIST_SIZE );
    dstRsp->routingTableListCount = srcRsp->routingTableListCount;
...
static bool zdpIsZigBeeStandardStatus(ZDO_Status_t bcStatus)
{
  ...
  {
    case ZDO_FAIL_STATUS:
    case ZDO_RESPONSE_WAIT_TIMEOUT_STATUS:
    case ZDO_CMD_COMPLETED_STATUS:
    case APS_NWK_ROUTE_DISCOVERY_FAILED_STATUS:
      return false;

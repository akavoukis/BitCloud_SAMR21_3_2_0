...
/** AF layer sequence number.

    Used when calling \ref IN_AfDataRequest_DataRequest.
*/
//static uint8_t s_afTransactionSequenceNumber = 0u;
...
static void ProcessAfIncomingData(APS_DataInd_t *dataInd);
    ...
    if ( (zclIncoming.hdr.frameControl.manufacturerSpecific) && ( zclIncoming.hdr.manufacturerCode != N_ZCL_MANUFACTURER_CODE ) )
    {
        ...
        else
        {
            ...
            if ( s_receivedAsUnicast )
            {
                ...
                IN_AfDataRequest_DataRequest(&apsBuffer->dataReq);
            }
...
void N_Zcl_Framework_EnableZclEndpoint_Impl(uint8_t endpoint)
{
    ...
    static const IN_AfIncomingData_Callback_t s_N_AfIncomingData_Callback =
    {
        ProcessAfIncomingData,
        -1
    };
    for ( uint8_t i = 0u; i < N_ZCL_FRAMEWORK_MAX_ENDPOINTS; i++ )
    {
        ...
        if ( s_enabledEndpoints[i] == 0u )
        {
            ...
            IN_AfIncomingData_Subscribe(endpoint, &s_N_AfIncomingData_Callback);

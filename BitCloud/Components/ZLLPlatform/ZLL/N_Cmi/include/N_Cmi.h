...
typedef struct N_Cmi_Callback_t
{
    ...
    */
    //void (*LeaveIndication)(NLME_LeaveInd_t* leaveIndication);
...
/** Indicates that a \ref N_Cmi_EnergyDetectScan has completed successfully.
*/
//typedef void (*N_Cmi_EnergyScanResult_t)(NLME_EDScanConfirm_t *EDScanConfirm, uint16_t transmissionFailures);
...
/** Perform a network discovery (active scan).
    ...
    \param durationPerChannel One of the BEACON_ORDER_... constants from NLMEDE.h
...
/** Do an energy detect scan
    \param channelMask The channelmask for the scan
    \param scanDuration The duration per channel
    \param pfDoneCallback The function to call after the scan is done
*/
//ZStatus_t N_Cmi_EnergyDetectScan(uint32_t channelMask, uint8_t scanDuration, N_Cmi_EnergyScanResult_t pfDoneCallback);

/** Retrieve the amount of network transmissions from the NIB */
uint16_t N_Cmi_GetTotalTransmissions(void);

/** Send a leave indication for this device
*/
void N_Cmi_SendLeaveIndication(bool rejoin, bool removeChildren);

/** Process a leave indication from another device. This should clean up the associated device table and the neighbor table

    \note This function is only implemented on a router
*/
//void N_Cmi_ProcessLeaveIndication(NLME_LeaveInd_t* leaveIndication);
...
/** Send AF data. Wrapper around afDataRequest but with N_Address_t* as the destination address instead of afAddrType_t*. */

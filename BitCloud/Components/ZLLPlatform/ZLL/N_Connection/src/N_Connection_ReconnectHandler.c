...
{
  if ((!reconnectMem.reconnectOngoing) && (!N_Connection_UpdateNetwork_IsBusy()))
...
static void nReconnectHandlerAddNeighbor(N_Beacon_t *pSelectedBeacon)
{
  ...
  neighbor = NWK_AddKnownNeighbor(pSelectedBeacon->sourceAddress, &extAddr);
...
static void nReconnectHandlerLeaveNetwork(bool removeParent)
{
  ...
  leaveControl.cleanAddresmap     = removeParent;
